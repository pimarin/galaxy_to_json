...
import ABRomicsonization.summarize
from ABRomicsonization.AbromicsResult import AbromicsResult
...
class hAMRonizedResultIterator(ABC):
    ...
    def hAMRonize(self, report_data, metadata):
        ...
        hAMRonized_result = AbromicsResult(**hAMRonized_result_data)
...
def generate_tool_subparser(subparser, analysis_tool):
    ...
    help = f"qbromics {analysis_tool}'s output report i.e., {report_file}"
    ...
    required_mandatory_metadata = ABRomicsonization._RequiredToolMetadata[analysis_tool]
...
def generic_cli_interface():
    ...
    parser = argparse.ArgumentParser(
        description="Convert ABRomics workflow results "
        "tool output(s) to "
        "hAMRonization specification"
        " format",
        prog="hamronize",
    ...
    if args.analysis_tool and args.analysis_tool != "summarize":
        ...
        for report_number, report in enumerate(args.report):
            parsed_report = ABRomicsonization.parse(report, metadata, args.analysis_tool)
        ...
        ABRomicsonization.summarize.summarize_reports(
