...
class IseScanIterator(AbromicsResultIterator):
    ...
    def parse(self):
        ...
        if self.metadata["orf_fna_file"]:
            self._check_input_files(self.abromics_results["orf_fna_file"])
        ...
        if self.metadata["orf_faa_file"]:
            self._check_input_files(self.abromics_results["orf_faa_file"])
            extracted_infos = self.__get_fasta_infos__()
        ...
        if self.metadata["is_fna_file"]:
            self._check_input_files(self.abromics_results["is_fna_file"])
            extracted_infos = self.__get_fasta_infos__()
