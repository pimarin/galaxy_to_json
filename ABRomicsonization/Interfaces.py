...
from ABRomicsonization import Summarize
...
class AbromicsResultIterator(ABC):
    ...
    def __init__(self, source, metadata):
        ...
        self.__check_input_files__(self.source)
        self.__abromics_structure__()

    def __check_input_files__(self, source):
        """
    ...
    def __abromics_structure__(self):
        """
    ...
    def write(self, report_list, total_report_count=1, output_location=None):
        """
...
def generate_tool_subparser(subparser, analysis_tool):
    ...
    """
    report_file = ABRomicsonization._ReportFileToUse[analysis_tool]
    description = (
    ...
    tool_parser.add_argument(
        "-o", "--output", default=None, help="Output location"
    )

    # any missing mandatory fields need supplied as CLI argument
    required_mandatory_metadata = ABRomicsonization._RequiredToolMetadata[
        analysis_tool
    ]
...
def generic_cli_interface():
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"%(prog)s {ABRomicsonization.__version__}",
    )

    # add tool specific parsers
    subparser = parser.add_subparsers(
        title="Tools with abromics reports", help="", dest="analysis_tool"
    )

    for analysis_tool in ABRomicsonization._RequiredToolMetadata.keys():
    ...
    summarize_subparser.add_argument(
        "abromics_reports", nargs="+", help="list of abromics reports"
    )
    ...
    if args.analysis_tool and args.analysis_tool != "summarize":
        ...
        for report in args.report:
            parsed_report = ABRomicsonization.parse(
                report, metadata, args.analysis_tool
            )
        ...
        export = Summarize.ReportSummary(
            report_list=args.abromics_reports, output_location=args.output
        )
