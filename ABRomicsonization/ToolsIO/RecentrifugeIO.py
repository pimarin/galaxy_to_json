...
class RecentrifugeIterator(AbromicsResultIterator):
    ...
    def __init__(self, source, metadata):
        ...
        self.rcf_stat_colnames = ["infos", "value"]
    ...
    def parse(self):
        ...
        if self.metadata["rcf_stat_file_path"]:
            self.__check_input_files__(self.metadata["rcf_stat_file_path"])
        ...
        if self.metadata["rcf_html_path"]:
            self.__check_input_files__(self.metadata["rcf_html_path"])
    ...
    def _filter_dataframe_type(
        ...
        reader = pd.read_table(
            handle, header=header_position, sep=separator, skiprows=skip
        )
        if col_names:
